...
class SimpleServer(BaseHTTPRequestHandler):
	...
	def do_POST(self):
		...
		if forward:
			forward_request(forward,post_data)

		self._set_response()
		self.wfile.write("POST request for {}".format(self.path).encode('utf-8'))


def forward_request(forward, data):
	try:
		response = requests.post(forward, data, timeout=0.0000000001)
...
if __name__ == '__main__':
	...
	parser.add_argument("-f", "--forward", help="forward incoming POST request toward a defined endpoint. \
	Define endpoint following the format => proto:ip:port (e.g., http://127.0.0.1:8000/ )", default=None)
